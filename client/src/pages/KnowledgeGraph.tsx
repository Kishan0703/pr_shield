...
/* ────────────────────────────────────────────────────────────────
 *  Module config — muted product palette
 * ──────────────────────────────────────────────────────────────── */

const MODULES: Record<ModuleId, ModuleConfig> = {
  frontend: { label: "Frontend", color: "#6b8fba", layer: 0, Icon: LayoutIcon },
  shared: { label: "Shared", color: "#8a7aa0", layer: 1, Icon: ComponentIcon },
  utilities: { label: "Utilities", color: "#87909a", layer: 1, Icon: Wrench },
  services: { label: "Services", color: "#5f9a9a", layer: 2, Icon: Cog },
  api: { label: "APIs", color: "#6f9f83", layer: 3, Icon: Network },
  backend: { label: "Backend", color: "#6c85a8", layer: 4, Icon: Server },
  database: { label: "Database", color: "#b48a5a", layer: 5, Icon: Database },
  config: { label: "Config", color: "#898b91", layer: 6, Icon: Settings },
  other: { label: "Other", color: "#777b84", layer: 6, Icon: Circle },
};
...
function buildDependencyGraph(
  ...
  const candidates = data.nodes
    .filter((n) => !hiddenModules.has(moduleByFileId.get(n.id) || "other"))
    .sort((a, b) => {
      const ia = importanceByFileId.get(a.id) || 0;
      const ib = importanceByFileId.get(b.id) || 0;
    ...
    .slice(0, MAX_DEPENDENCY_NODES);

  const visibleIds = new Set(candidates.map((n) => n.id));
  const nodes: FGNode[] = candidates.map((n) => ({
    id: n.id,
    label: n.label,
    path: n.path,
    kind: "file",
...
export default function KnowledgeGraph() {
  ...
  useEffect(() => {
    ...
    const loadGraph = async () => {
      ...
      setError(null);
      try {
        const res = await api.get<GraphResponse>(`/api/repos/${repoId}/graph`);
  ...
  const drawNode = useCallback(
    (node: FGNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      if (isCluster && !isFolderTreeCluster) {
        ...
        ctx.fillStyle = dim ? "rgba(19, 22, 28, 0.7)" : "rgba(23, 27, 34, 0.98)";
        ctx.fill();
        ...
        roundRect(ctx, x - w / 2, y - h / 2, 5, h, 12);
        ctx.fill();
        ...
        ctx.fillStyle = "rgba(190,190,184,0.78)";
        ctx.fillText(
        ...
        ctx.fillStyle = dim ? hexToRgba(baseColor, 0.42) : hexToRgba(baseColor, 0.92);
        ctx.fill();
  ...
  const linkColor = useCallback(
    (link: FGLink) => {
      ...
      if (dim) return "rgba(148, 163, 184, 0.06)";
      if (onPath) return "rgba(245, 245, 244, 0.82)";
      ...
      if (link.type === "contains") return "rgba(180,180,176,0.24)";
      return "rgba(180,180,176,0.2)";
    },
  ...
  return (
    <div
      className="fixed inset-0 bg-[#101216] text-stone-200 overflow-hidden"
    ...
    >
      ...
      {/* Top bar */}
      <div className="absolute top-0 left-0 right-0 z-30 h-12 border-b border-white/[0.08] bg-[#11141a]/88 backdrop-blur flex items-center px-4 gap-3">
        <Link
          to="/dashboard/repos"
      ...
      {/* Left sidebar */}
      <aside className="absolute top-12 left-0 bottom-0 w-64 z-20 border-r border-white/[0.08] bg-[#12161c]/94 backdrop-blur flex flex-col">
      ...
      {hoverNode && (
        <div
          className="pointer-events-none fixed z-40 px-3 py-2 rounded-md bg-[#171b22] border border-white/15 shadow-xl text-[11px]"
          style={{
      ...
      <div className="absolute bottom-4 right-4 z-20 flex items-center gap-0.5 rounded-md bg-[#171b22]/96 border border-white/10 p-0.5 shadow-lg">
      ...
      {showMinimap && graphData.nodes.length > 0 && (
        <div
          className="absolute bottom-4 right-[calc(1rem+260px)] z-20 rounded-md bg-[#171b22]/96 border border-white/10 p-1.5 shadow-lg"
      ...
      {showShortcuts && (
        ...
        >
          <div
            ...
            className="rounded-lg bg-[#171b22] border border-white/10 p-5 w-[420px] shadow-2xl"
          >
      ...
      {selectedNode && (
        <div
          className="absolute top-12 right-0 bottom-0 w-[380px] z-30 border-l border-white/[0.08] bg-[#12161c]/96 backdrop-blur flex flex-col"
...
function ArchitectureColumns({
  ...
  return (
    <>
      ...
      {/* Header rail */}
      <div className="absolute top-12 left-64 right-0 h-12 z-10 border-b border-white/[0.08] flex pointer-events-none bg-[#11141a]/88 backdrop-blur">
        {layers.map(([layer, mods]) => {
          const accent = MODULES[mods[0]].color;
          return (
            <div
...
function Status({
  ...
  return (
    <div className="min-h-screen bg-[#101216] text-zinc-200 flex flex-col items-center justify-center gap-3 p-6">
      {tone === "error" ? (
